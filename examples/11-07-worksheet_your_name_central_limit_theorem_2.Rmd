...
```{r}
number_of_samples <- your_answer                   #change this
number_of_observations_in_sample <- your_answer    #change this
sample_means <- replicate(number_of_samples, table(sample(population, number_of_observations_in_sample))["success"])
sample_means <- sample_means / number_of_observations_in_sample
hist(sample_means, main="Sample Size: 50", xlab="Sample Means")
print(paste("The mean with", number_of_samples, "is", mean(sample_means)))
...
p_hat - 1.96 * your_answer
p_hat + 1.95 * your_answer
```
...
(b) Change the seed number or delete the line and execute the above three code blocks a few times to get an example where the population proportion does not fall in the confidence interval. What was your point estimate and interval? (Your PDF output may not contain this number.). 
  answer:
...
(c) How many times did you need to run it for part (b)?  
  answer:
