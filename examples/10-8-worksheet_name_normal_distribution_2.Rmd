...
- Students lose one point if Q0 is unanswered 

---

### Goal

- Learning the function that computes the percentile from x or its Z-score.
- Learning the function that computes the Z-score or x from the percentile.
...
When the Z-score is zero, then it splits the graph equally into two pieces.
...
We need to find the Z-score z such that $P(Z < z) = 0.8$, here $0.8 = 80/100$. 
We can use the `qnorm` for this. The syntax is the same as `qnorm` but the first 
...
For the purpose of Math 2265, students are expected to know how conversion works both ways (x to Z-score and Z-score to x).
